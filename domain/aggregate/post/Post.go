...
func NewPostDraft(pub common.DomainEventPublisher, cmd NewPostDraftCommand) (Post, error) {
	pid := GenPId.NextID()
	if err := pub.Publish(common.PostCreatedEvent, PostCreatedEvent{
		Uid: cmd.Uid,
		Pid: pid,
	}); err != nil {
		return nil, err
...
func (p *PostImpl) Update(pub common.DomainEventPublisher, cmd UpdatePostCommand) error {
	...
	p.Slug = cmd.Slug
	p.Content = ""
	return pub.Publish(common.PostUpdatedEvent, PostUpdatedEvent{
		Uid: cmd.Uid,
		Pid: p.Pid,
	})
}
...
func (p *PostImpl) Delete(pub common.DomainEventPublisher, cmd DeletePostCommand) error {
	...
	return pub.Publish(common.PostDeletedEvent, PostDeletedEvent{
		Uid: cmd.Uid,
		Pid: p.Pid,
	})
}
