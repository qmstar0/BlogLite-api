...
func CreateUser(pub common.DomainEventPublisher, cmd CreateUserCommand) (User, error) {
	uid := GenUId.NextID()
	if err := pub.Publish(common.UserCreatedEvent, UserCreatedEvent{
		Uid:   uid,
		Email: cmd.Email,
	}); err != nil {
		return nil, err
...
func (u *UserImpl) ResetPassword(pub common.DomainEventPublisher, cmd UserResetPassowrdCommand) error {
	...
	return pub.Publish(common.UserResetPasswordEvent, UserPasswordResetedEvent{
		Uid:         u.Uid,
...
func (u *UserImpl) Update(pub common.DomainEventPublisher, cmd UpdateUserCommand) error {
	u.Name = cmd.Name
	return pub.Publish(common.UserUpdatedEvent, UserUpdatedEvent{
		Uid:  u.Uid,
		Name: cmd.Name,
	})
}
...
func (u *UserImpl) Delete(pub common.DomainEventPublisher, cmd DeleteUserCommand) error {
	...
	return pub.Publish(common.UserDeletedEvent, UserDeletedEvent{
		Uid:   u.Uid,
		Email: cmd.Email,
	})
}
